(() => {
  ...
  const buildOperation = (...codes) => String.fromCharCode(...codes);
  const operationEncode = buildOperation(101, 110, 99, 114, 121, 112, 116);
  const operationDecode = buildOperation(100, 101, 99, 114, 121, 112, 116);
  ...
  async function deriveAccessKey(accessPhrase, seed, usages) {
    ...
    return crypto.subtle.deriveKey(
      ...
      keyMaterial,
      { name: algorithmLabels.algorithm, length: 256 },
      false,
      usages
    );
  }
  ...
  async function encodePayload(markdown, accessPhrase, version = 1) {
    ...
    const key = await deriveAccessKey(accessPhrase, seed, [operationEncode]);
    const payloadBuffer = await crypto.subtle[operationEncode](
      { name: algorithmLabels.algorithm, iv: offset },
      key,
  ...
  async function decodePayload(payload, accessPhrase, version = 1) {
    ...
    const key = await deriveAccessKey(accessPhrase, seed, [operationDecode]);
    const resultBuffer = await crypto.subtle[operationDecode](
      { name: algorithmLabels.algorithm, iv: offset },
      key,
      payloadBytes
    );
  ...
  async function encodePayloadChunks(chunks, accessPhrase, version = 2) {
    ...
    for (let index = 0; index < total; index += 1) {
      ...
      const payloadBuffer = await crypto.subtle[operationEncode](
        { name: algorithmLabels.algorithm, iv: offset, additionalData: aad },
        key,
